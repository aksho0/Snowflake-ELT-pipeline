...
-- Gold Layer --
    Dimesion Tables -->
        ...
        5. dim_supplier
    Fact Tables -->
        1. fct_sales (One row per POS transaction)
        2. fct_daily_sales (One row per date + store + category)
        3. fct_purchase_orders (One row per ERP purchase order)
        4. fct_store_category_margin (One row per sales date + store + category)
        5. fct_iot_sensor_readings (One row per sensor reading)
